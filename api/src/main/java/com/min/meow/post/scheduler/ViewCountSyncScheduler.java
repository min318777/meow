...
public class ViewCountSyncScheduler {
    ...
    public void syncViewCountsToDatabase() {
        ...
        try {
            ...
            // 자기호출 방지: Redis 수집(트랜잭션 불필요)과 DB 반영(@Transactional)을 분리
            var deltas = viewCountService.collectFromRedis();
            if (!deltas.isEmpty()) {
                viewCountService.applyDeltasToDb(deltas);
                // DB 반영 후 Sorted Set 점수도 갱신 (조회수 × 1)
