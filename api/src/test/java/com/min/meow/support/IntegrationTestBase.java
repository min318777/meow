 ...
 * dependency_overrides[get_current_user] → JwtUtil.createAccessToken() + X-Auth-Version: v2
 * test_session.add(user)              →   userRepository.save(user)
 * function scope fixture              →   @AfterEach에서 직접 삭제
 * </pre>
 *
 * <h3>왜 X-Auth-Version: v2인가</h3>
 * JwtFilter v1(기본)은 토큰의 userId로 DB에서 User를 조회한다.
...
public abstract class IntegrationTestBase {
     ...
     *
     * X-Auth-Version: v2를 포함하여 JwtFilter가 DB 조회 없이 JWT Claims로만 인증하도록 한다.
     ...
     * @param loginId 로그인 ID (JWT 클레임에 포함)
     * @return Authorization: Bearer {token} + X-Auth-Version: v2 헤더
     */
    protected HttpHeaders createAuthHeader(Long userId, String role, String loginId) {
        // 실제 JwtUtil로 서명된 토큰 생성 — 운영과 동일한 로직
        String token = jwtUtil.createAccessToken(userId, role, loginId, List.of());
